...
4. Start `dist/index.js` with a temporary `BOT_DB_PATH`, generated 32-byte `BOT_SECRET_KEY`, local `FLOWAY_BASE_URL`, and non-production credentials. Drive `/bind`, `/usage`, `/quota verbose`, and the hidden quota-window diagnostic command.
...
7. For notifier changes, bind through the command surface, let the first selected provider quota-window observation seed the disposable cursor silently, then supply two matching advanced observations and capture the automatic Telegram message. Include a primary/secondary slot relabel with unchanged normalized facts and verify it creates no event.
