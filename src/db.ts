...
const DISCARDABLE_STATE_COLUMNS: readonly ColumnSignature[] = [
...
const DISCARDABLE_NOTIFICATION_COLUMNS: readonly ColumnSignature[] = [
  { name: 'telegram_user_id', type: 'TEXT', notnull: 1, pk: 1 },
  { name: 'upstream_id', type: 'TEXT', notnull: 1, pk: 2 },
...
export class BindingStore {
  ...
  private migrateVersionZero(): void {
    this.immediateTransaction(() => {
      ...
      const legacyRows = tableNames.includes('bindings')
        ? this.readAcceptedLegacyBindings()
        : [];
      const migrationTimeMs = Date.now();
      if (tableNames.includes('bindings')) {
        if (tableNames.includes('__bindings_v0_migration')) {
          throw new Error('Reserved migration table already exists');
        }
        this.db.exec('ALTER TABLE bindings RENAME TO __bindings_v0_migration');
      }

      createVersionOneSchema(this.db);
      ...
      this.verifyMigratedBindings(legacyRows, migrationTimeMs);
      if (tableNames.includes('bindings')) this.db.exec('DROP TABLE __bindings_v0_migration');

      this.dropOnlyMatchingTable('primary_window_state', DISCARDABLE_STATE_COLUMNS);
      this.dropOnlyMatchingTable('primary_window_notification', DISCARDABLE_NOTIFICATION_COLUMNS);
      this.verifyCurrentSchema(false);
      this.db.exec(`PRAGMA user_version = ${CURRENT_SCHEMA_VERSION}`);
      if (this.pragmaInteger('user_version') !== CURRENT_SCHEMA_VERSION) {
        throw new Error('Failed to persist database schema version');
      }
    });
  }

  private readAcceptedLegacyBindings(): LegacyBindingRow[] {
    if (!schemaMatches(this.tableInfo('bindings'), LEGACY_BINDING_COLUMNS) || !this.hasTableFlags('bindings', 0, 0)) {
  ...
  private verifyMigratedBindings(before: readonly LegacyBindingRow[], migrationTimeMs: number): void {
    ...
    for (let index = 0; index < before.length; index += 1) {
      ...
      if (!left || !right
        ...
        || right.bound_at_ms !== migrationTimeMs
        || right.updated_at_ms !== migrationTimeMs) {
        throw new Error('Binding values changed during migration');
      }
    }
  }

  private verifyCurrentSchema(requireVersion = true): void {
    if (requireVersion && this.pragmaInteger('user_version') !== CURRENT_SCHEMA_VERSION) {
      throw new Error('Database schema version does not match the current schema');
    }
    for (const table of ['bindings', ...NEW_TABLE_NAMES]) {
      if (!this.tableNames().includes(table)) throw new Error(`Required table ${table} is missing`);
      if (!this.hasTableFlags(table, 0, 1)) throw new Error(`Required table ${table} is not STRICT`);
    }
    const foreignKeyProblems = this.db.prepare('PRAGMA foreign_key_check').all();
    if (foreignKeyProblems.length > 0) throw new Error('Database contains foreign-key violations');
    const deliveryForeignKeys = this.db.prepare('PRAGMA foreign_key_list(primary_window_delivery)').all() as unknown as Array<{ table: unknown; on_delete: unknown }>;
  ...
  private tableNames(): string[] {
    return (this.db.prepare("SELECT name FROM sqlite_schema WHERE type = 'table'").all() as unknown as Array<{ name: string }>)
      .map(row => row.name);
  }
  ...
  private hasTableFlags(table: string, withoutRowId: number, strict: number): boolean {
    const row = (this.db.prepare('PRAGMA table_list').all() as unknown as TableListRow[]).find(candidate => candidate.name === table);
    return row?.wr === withoutRowId && row.strict === strict;
  }

  private dropOnlyMatchingTable(table: string, signature: readonly ColumnSignature[]): void {
    if (!this.tableNames().includes(table)) return;
    if (schemaMatches(this.tableInfo(table), signature) && this.hasTableFlags(table, 0, 0)) {
