...
import { QuotaWindowNotifier } from './quota-window-notifier.js';
...
type TelegramCallSignal = NonNullable<NonNullable<Parameters<typeof bot.telegram.callApi>[2]>['signal']>;
  ...
  telegram: {
    async sendMessage(chatId, text, signal) {
      await bot.telegram.callApi('sendMessage', {
        ...
        text,
        parse_mode: 'HTML',
      }, { signal: signal as TelegramCallSignal });
    },
  },
...
const shutdown = async (signal: string): Promise<void> => {
  ...
  const notifierStopped = quotaWindowNotifier.stop();
